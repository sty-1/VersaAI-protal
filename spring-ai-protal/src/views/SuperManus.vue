    ...
    <div class="chat-container">
      ...
      <div class="input-area">
        <div class="input-row">
          ...
          <button
            class="send-btn"
            @click="isRunning ? stopTask() : startTask()"
...
import {
  SparklesIcon, PaperAirplaneIcon, StopIcon,
  WrenchScrewdriverIcon, CheckCircleIcon, ExclamationCircleIcon
...
const handleEvent = (evt) => {
  ...
  switch (evt.type) {
    ...
    case 'tool_call': {
      let tool = '', args = ''
      try {
        const payload = typeof evt.content === 'string' ? JSON.parse(evt.content) : evt.content
        tool = payload.tool || ''
        args = payload.args || ''
      } catch {
        tool = evt.content || ''
    ...
    case 'tool_result':
      removeLastThinking()
      timeline.value.push({ role: 'assistant', type: 'tool_result', content: evt.content || '' })
      break
...
const removeLastThinking = () => {
  ...
  if (len > 0 && timeline.value[len - 1].type === 'thinking') {
    timeline.value.pop()
  }
}
...
const startTask = async () => {
  ...
  try {
    const url = `http://localhost:8080/ai/manus/chat?message=${encodeURIComponent(prompt)}`
    console.log('[SuperManus] Fetching:', url)
    ...
    console.log('[SuperManus] Response status:', response.status, 'ok:', response.ok)
    ...
    while (true) {
      ...
      if (done) {
        console.log('[SuperManus] Stream ended, flushing buffer:', JSON.stringify(buffer))
      ...
      console.log('[SuperManus] Raw chunk:', JSON.stringify(chunk))
      buffer += chunk
    ...
    if (error.name === 'AbortError') {
      console.log('[SuperManus] Aborted by user')
    } else {
      console.error('[SuperManus] Fetch error:', error)
      timeline.value.push({
        role: 'assistant',
        type: 'error',
...
const parseSSELines = (text) => {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      console.warn('[SuperManus] Failed to parse SSE data:', jsonStr, e)
    }
  }
}
...
.thinking-avatar {
  ...
  .avatar-icon { width: 18px; height: 18px; color: hsl(30, 90%, 50%); }
}
...
.tool-card {
  ...
  .tool-name {
    font-size: var(--font-xs); padding: 1px 8px;
  ...
  .tool-card-body {
    ...
    font-size: var(--font-sm); color: var(--text-secondary); line-height: 1.6;
  }
  .tool-args {
  ...
  .result-content {
    :deep(p) { margin-bottom: var(--space-1); }
    :deep(code) { background: var(--surface-page); padding: 1px 6px; border-radius: 3px; font-size: 0.9em; }
    :deep(pre) { background: var(--surface-page); padding: var(--space-2); border-radius: var(--radius-sm); overflow-x: auto; }
  }
}
...
.input-area {
  ...
  .task-input {
    ...
    font-size: var(--font-sm); font-family: inherit; line-height: 1.6;
    resize: none; outline: none; max-height: 160px;
    transition: border-color var(--transition-base);
    &:focus { border-color: hsl(30, 90%, 50%); }
  ...
  .send-btn {
    ...
    background: linear-gradient(135deg, hsl(30, 95%, 55%), hsl(20, 90%, 48%));
    color: #fff; cursor: pointer; flex-shrink: 0;
    transition: all var(--transition-base);
    .btn-icon { width: 20px; height: 20px; }
    &:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 4px 16px rgba(249, 115, 22, 0.35); }
    &:disabled { opacity: 0.4; cursor: not-allowed; }
    &.is-stop { background: var(--color-error); animation: pulseStop 2s ease-in-out infinite; }
  }
  .input-hint {
    font-size: var(--font-xs); color: var(--text-muted);
...
@media (max-width: 768px) {
  .messages { padding: var(--space-4) var(--space-4) var(--space-2); }
